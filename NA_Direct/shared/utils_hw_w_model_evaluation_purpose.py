...
def build_tot_stats(comp_out):
    ...
    comp_out_save = comp_out.copy()#for debugging purpose
    stats = DataFrame()
    grp = comp_out.groupby('quoteid')
    #a_path = '~/Box Sync/copra_hw_jp_py3_merge_updated_function/copra_hw-py3_port/debug/'
   #comp_out.to_csv(a_path + 'comp_out.csv', index=False)#added by Bonnie for debugging purpose on June 7th, 2018
    #.to_csv('./debug/grp.csv') #added by Bonnie for debugging purpose on June 7th, 2018
    
    stats['quoted_price'] = grp['quoted_price'].sum().round(decimals=2) #modified by BB on June 7th, 2018
    ...
    #stats['price_act] = grp['price_act'].sum().round(decimals=0)
    ...
    return stats#, comp_out_save #added for debugging purpose by BB


def build_output(comp, source):
    comp_out = build_comp_out(comp)

    stats = build_tot_stats(comp_out) #added by BB for debugging purpose
    
    c_out = spread_optimal_price(comp_out, stats)#modified for debugging by BB

    if source == 'online':
        stats = stats.iloc[0]  # convert to Series object, as expected by CIO

    return c_out,  stats#, comp_out_save #added by BB for debugging purpose
...
def spread_optimal_price(quote_df, tot_deal_stats):
    ...
    quote_df1 = quote_df.copy()

    #train_vs.to_csv('./debug/train_vs_test_quant.csv')
    ...
    tots = merge(tots, tot_deal_stats.reset_index()[['bot_OP', 'quoted_price','quoteid']], on='quoteid', how='inner')#Added by BB for debugging purpose on June 7th, 2018
    ...
    # build mask
    mask = tots.apply(lambda row: Series([((row[cols[0]] < row['bot_OP']) & (row['bot_OP'] <= row[cols[1]]))*1 for cols in mask_cols]), axis=1)#modified by BB on June 8th, 2018
    ...
    #tots_debug = tots.copy() #added by Bonnie for debugging purpose on June 7th, 2018

    tots['alpha'] = ((tots['bot_OP'] - tots['spread_low'])/(tots['spread_high'] - tots['spread_low'])).fillna(0.)
    #tots['alpha2'] = ((tots['quoted_price'] - tots['spread_low'])/(tots['spread_high'] - tots['spread_low'])).fillna(0.)
    ...
    #adj_spread_price['spread_price_quoted'] = adj_spread_price['spread_low'] + \
                #   (adj_spread_price['spread_high'] - adj_spread_price['spread_low']) * adj_spread_price['alpha2']
                   
    # This section loads the spread optimal prices to the quote_df dataframe
    quote_df = quote_df.set_index(['quoteid', 'componentid'])
    quote_df['bot_spread_OP'] = adj_spread_price.set_index(['quoteid', 'componentid'])['spread_price'] #commented out by BB for model evaluation purpose
    
   #############################################################below section is for JP model review purpose ##########
    # build mask
    spread_price = quote_df1.loc[:, ['quoteid', 'componentid', 'adj_price_L', 'adj_price_M', 'adj_price_H', 'list_price']]
    ...
    mask = tots.apply(lambda row: Series([((row[cols[0]] < row['quoted_price']) & (row['quoted_price'] <= row[cols[1]]))*1 for cols in mask_cols]), axis=1)#modified by BB on June 8th, 2018
    mask['quoteid'] = tots['quoteid']
    ...
    tots['alpha2'] = ((tots['quoted_price'] - tots['spread_low'])/(tots['spread_high'] - tots['spread_low'])).fillna(0.)
    adj_spread_price = merge(adj_spread_price, tots[['quoteid', 'alpha2']], on='quoteid', how='left')
    adj_spread_price['spread_price_quoted'] = adj_spread_price['spread_low'] + \
                   (adj_spread_price['spread_high'] - adj_spread_price['spread_low']) * adj_spread_price['alpha2']
    
    quote_df['bot_spread_QP'] = adj_spread_price.set_index(['quoteid', 'componentid'])['spread_price_quoted']    
    
    #added above by BB for spreading based on quoted price on June 7th, 2018
        

    return quote_df#, tots_debug
