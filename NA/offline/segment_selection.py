...
from offline.variable_selection import main_regression
...
def compute_metrics(data, reg_vals, coef_lev, layer, idx_fold=True):
    ...
    metrics_dict = dict((key + '_' + func_dict[key][0][i], func_dict[key][1][i]) for key in list(func_dict.keys()) for i in
                        range(len(func_dict[key][0])))

    metrics_func = dict((key, func_dict[key][0]) for key in list(func_dict.keys()))
