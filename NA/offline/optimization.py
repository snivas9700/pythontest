...
from numpy import isnan, exp, array, ceil, array_split, reshape, tile, expand_dims, transpose, where, argmax
from pandas import merge, DataFrame

from offline.method_defs import opt_method
...
def optprice_mtier(data, coef):
    opt_key = list(opt_method.keys())[0]
...
def run_search(m, res, b0, b1, op_fail, rev_adj, level, max_levels, max_search=None):
    ...
    if level < max_levels:  # check that the number of searches run (level) is less than max allowed
        ...
        max_search = search[list(range(N)), max_idx].reshape(-1, 1)  # [N x 1]
