...
def run_quants_offline(data, idx_cols, in_feats, out_feat, grp_cols):
    ...
    if len(grp_cols) > 0:
        ...
        for t3, grp in data.groupby(grp_cols):
            ...
            if train.shape[0] > len(in_feats):
                full_rank = True  # assume the design matrix is full rank
                ...
                test.loc[:, 'winloss'] = 1
                train.to_csv('./debug/quants_training_grp.csv') # for debug
                try: # if design matrix is not full rank, quantile regression will throw a ValueError 
    ...
    q_mods.to_csv('./debug/q_mods.csv')
...
#        q_res = quant_reg_pred(test_, in_feats, q_mods.xs('ALL', level='segment_id'), qs_.index)
