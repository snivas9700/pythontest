...
try:
    ...
    wd = '~/PycharmProjects/copra_hw_port36'
...
config = {'model':
              {'fe': 'discount ~ 1 + comcostpofl + comloglistprice + com_contrib + upgmes + '
                     'ln_ds + indirect + comdelgpricel4pofl + endofqtr + ufc_incl + winloss + n_sbc'
                , 're': '(comcostpofl || lvl4/lvl3/lvl2/lvl1) + (comcostpofl || crmindustryname/crmsectorname)'
               }
            , 'wr_feats': parse_regmethod(regmethod)[1]
            , 'quant_feats': {'in_feats': ['quant_intercept', 'ln_ds', 'ln_vs'], 'target': 'metric'}
            , 'uplift': UPLIFT
            , 'alpha': ALPHA
...
def run_quant_testing(data, models, idx_cols, grp_cols, uplift=1., alpha=0.0):
    ...
    for s_id, grp in data.groupby(grp_cols):
        ...
        # mix raw quantiles + quantile regressions
        quants = q_res.apply(lambda row: bound_quant(row, qs, grp_cols), axis=1)
...
# for region in ['NA_newmap_4lvl_DirectOnly']:
#for region in ['NA_newmap_4lvl_DirectOnly_non_SAPHANA']:
#for region in ['NA_direct_nonSAPHANA_apprved']:
for region in ['NA_direct_nonSAPHANA_apprved_quant_intercept']:
    ...
    train_pn = prep_comp(train_data, sbc_map)
    t = dt.now()
...
#    print('Adding Intercept column to the design matrix...')
#    train_qt['quant_intercept'] = 1 # add quantile regression intercept column
...
#    print('Adding Intercept column to the design matrix...')
#    train_vs['quant_intercept'] = 1 # add quantile regression intercept column
