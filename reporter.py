...
class Reporter:
    ...
    def calculate_total_deal_stats(self, quote_df, COP_h=0, COP_m=0, COP_l=0):
        ...
        if total_deal_stats['DealAdjLowPrice'] == 0:
            ...
            total_deal_stats['DealBotLineOptimalPrice'] = BPF.OptPrice(
                ...
                0, 0, 0)
            #Japan specific changes for DealBotLineOptimalPrice lower bound capping based on value score
            if (quote_df['Countrycode'][0] == 'JP'):
                deal_value_score  = quote_df['value_score'].sum()
                if (quote_df['Indirect(1/0)'].values[0]==1):
                    if ( total_deal_stats['DealBotLineOptimalPrice'] < 0.8 * deal_value_score):
                        total_deal_stats['DealBotLineOptimalPrice'] = 0.8 * deal_value_score
                elif (quote_df['Indirect(1/0)'].values[0]==0):
                    if ( total_deal_stats['DealBotLineOptimalPrice'] < 0.9 * deal_value_score):
                        total_deal_stats['DealBotLineOptimalPrice'] = 0.9 * deal_value_score
        ...
        elif ('GEO_CODE' in quote_df and quote_df['GEO_CODE'][0] == 'EMEA'):
            total_deal_stats['DealBotLineOptimalPriceIntervalLow'] = quote_df['OptimalPriceIntervalLow'].sum()
            total_deal_stats['DealBotLineOptimalPriceIntervalHigh'] = quote_df['OptimalPriceIntervalHigh'].sum()
        else:
    ...
    def TSS_calculate_total_deal_stats(self, quote_df, COP_h=0, COP_m=0, COP_l=0):
        quote_df_hw=quote_df.copy()
        quote_df_tss=quote_df.copy()
        
        quote_df = quote_df_hw.drop_duplicates(subset=['Componentid'],keep='first')
        tss_df = quote_df_tss[quote_df_tss.ParentMapping_ComponentID.notnull()]
        tss_df = quote_df_tss[quote_df_tss.ParentMapping_ComponentID != '']
            
        #this section calculates the total deal values
        total_deal_stats = pd.Series('', index=['  General Total Quote Data'])
        total_deal_stats['DealListPrice'] = quote_df['ComListPrice'].sum() + tss_df['totalcharge'].sum()
        total_deal_stats['DealSize'] = quote_df['ComListPrice'].sum() + tss_df['totalcharge'].sum()
        total_deal_stats['DealTMC'] = quote_df['ComTMC'].sum() + tss_df['Cost'].sum()
        #print '*PredictedQuotePrice: ',quote_df['PredictedQuotePrice']

        total_deal_stats['DealPredictedQuotePrice'] = quote_df[
            'PredictedQuotePrice'].sum() + tss_df['TSS_OptimalPrice'].sum()
        #  this section contains Price Range Data (Line Item Sum)
        total_deal_stats['  Price Range Data (Line Item Sum)'] = ''
        total_deal_stats['DealAdjLowPrice'] = quote_df['AdjComLowPrice'].sum() + tss_df['TSS_AdjComLowPrice'].sum()
        total_deal_stats['DealAdjMedPrice'] = quote_df['AdjComMedPrice'].sum() + tss_df['TSS_AdjComMedPrice'].sum()
        total_deal_stats['DealAdjHighPrice'] = quote_df['AdjComHighPrice'].sum() + tss_df['TSS_AdjComHighPrice'].sum()
        #  this section contains Quoted Price Data (Line Item Sum)
        total_deal_stats['  Quoted Price Data (Line Item Sum)'] = ''
        total_deal_stats['DealQuotePrice'] = quote_df['ComQuotePrice'].sum() + tss_df['TSS_OptimalPrice'].sum()        
        total_deal_stats['DealQuotePriceWinProb'] = ''
        total_deal_stats[
            'DealQuotePriceGP'] = total_deal_stats['DealQuotePrice'] - total_deal_stats['DealTMC']
        total_deal_stats['DealQuotePriceExpectedGP'] = quote_df[
            'QuotePriceExpectedGP'].sum() + tss_df['TSS_OptimalPriceExpectedGP'].sum()
        if total_deal_stats['DealQuotePriceGP'] == 0 :
            total_deal_stats['DealQuotePriceWinProb'] = 0
        else :
            total_deal_stats['DealQuotePriceWinProb'] = total_deal_stats[
        ...
        #  this section contains optimal price data
        total_deal_stats['  Optimal Price Data (Line Item Sum)'] = ''
        total_deal_stats['DealOptimalPrice'] = quote_df['OptimalPrice'].sum() + tss_df['TSS_OptimalPrice'].sum()
        total_deal_stats['DealOptimalPriceWinProb'] = ''
        total_deal_stats['DealOptimalPriceGP'] = quote_df[
            'OptimalPriceGP'].sum() + tss_df['TSS_OptimalPriceGP'].sum()
        total_deal_stats['DealOptimalPriceExpectedGP'] = quote_df[
            'OptimalPriceExpectedGP'].sum() + tss_df['TSS_OptimalPriceExpectedGP'].sum()
        total_deal_stats['DealOptimalPriceWinProb'] = 0  #
        if total_deal_stats['DealOptimalPriceGP'] != 0:  #
            total_deal_stats['DealOptimalPriceWinProb'] = total_deal_stats[
                'DealOptimalPriceExpectedGP'] / total_deal_stats[
            ...
            total_deal_stats['DealOptimalPriceWinProb'] = 0
        total_deal_stats['DealOptimalPriceIntervalLow'] = quote_df[
            'OptimalPriceIntervalLow'].sum() + tss_df['TSS_OptimalPriceIntervalLow'].sum()
        total_deal_stats['DealOptimalPriceIntervalHigh'] = quote_df[
            'OptimalPriceIntervalHigh'].sum() + tss_df['TSS_OptimalPriceIntervalHigh'].sum()
        #  this section contains Quoted Price Data (Bottom-Line)
        total_deal_stats['  Quoted Price Data (Bottom-Line)'] = ''
        total_deal_stats['DealBotLineQuotePrice'] = total_deal_stats[
            'DealQuotePrice']
        ...
        if total_deal_stats['DealAdjLowPrice'] == 0:
            ...
            total_deal_stats['DealBotLineOptimalPrice'] = quote_df['OptimalPrice'].sum() + tss_df['TSS_OptimalPrice'].sum()
            '''
            total_deal_stats['DealBotLineOptimalPrice'] = BPF.OptPrice(
                total_deal_stats['DealAdjLowPrice'],
                total_deal_stats['DealAdjMedPrice'],
        ...
        if total_deal_stats['DealAdjLowPrice'] == 0:
            ...
            total_deal_stats['DealBotLineOptimalPriceIntervalLow'] = quote_df['OptimalPriceIntervalLow'].sum() + \
                                                                     tss_df['TSS_OptimalPriceIntervalLow'].sum()
            total_deal_stats['DealBotLineOptimalPriceIntervalHigh'] = quote_df['OptimalPriceIntervalHigh'].sum() + \
                                                                      tss_df['TSS_OptimalPriceIntervalHigh'].sum()
            '''
            total_deal_stats['DealBotLineOptimalPriceIntervalLow'], total_deal_stats[
                'DealBotLineOptimalPriceIntervalHigh'] = BPF.OptPriceConfIntervl(
                    total_deal_stats['DealBotLineOptimalPrice'],
    ...
    #reporter columns considered for TSS(REPORTER_COLUMNS_TSS), merge by componentid and TSScompid
    def report(self, price_df, zero_price_df, ComRevDivCd_orgi,ComRevDivCd_orgi2, *quote_dfs):
        ...
        if 'TSSComponentincluded' not in quote_df_out_0 or quote_df_out_0['TSSComponentincluded'][0] == 'N':                        
            total_deal_stats_0 = self.calculate_total_deal_stats(
                quote_df_out_0, 0, 0, 0)
        elif quote_df_out_0['TSSComponentincluded'][0] == 'Y':
            total_deal_stats_0 = self.TSS_calculate_total_deal_stats(
                quote_df_out_0, 0, 0, 0)
            
        quote_df, total_deal_stats1, PricePoint_data1 = self.dataclean(
            quote_df_out_0, total_deal_stats_0, price_df)
        
        #quote_df_out1 = pd.DataFrame(quote_df, columns = REPORTER_COLUMNS_TSS )
        quote_df_out1 = pd.DataFrame(quote_df)  
        quote_df_out1 = quote_df_out1.rename(columns={'geo_code': 'GEO_CODE'})
        quote_df_out1 = quote_df_out1.loc[:,~quote_df_out1.columns.duplicated()]
        quote_df_out1['GEO_CODE'] = quote_df_out1['GEO_CODE'].fillna(method='ffill').fillna(method='bfill')
        
        if (not GeoDispatcher().is_EMEA_old(quote_df_out1)) and quote_df_out1['TSSComponentincluded'][0] == 'Y':
            fillna_values = ['TSSComponentincluded','TSSContstartdate','TSSContenddate',
                             'TSSPricerefdate','TSSPricoptdescription','TSSFrameOffering',
                             'ChangePeriodStopDate','TSSContduration','ctry_desc']
            quote_df_out1[fillna_values] = quote_df_out1[fillna_values].fillna(method='ffill').fillna(method='bfill')
            quote_df_out1.sort_values('Componentid', inplace=True)
            quote_df_out1['TSScomid'] = quote_df_out1['TSScomid'].fillna(0)
            quote_df_out1['ComRevDivCd'] = pd.DataFrame(ComRevDivCd_orgi.split('-'))            
            del quote_df_out1['ComRevDivCd']
            quote_df_out1 = pd.merge(quote_df_out1,ComRevDivCd_orgi2, on=['Componentid','TSScomid'])
            quote_df_out1 = pd.DataFrame(quote_df_out1, columns = REPORTER_COLUMNS_TSS)
        elif quote_df_out1['GEO_CODE'][0] == 'JP':
            ...
            quote_df_out1 = pd.DataFrame(quote_df_out1, columns = REPORTER_COLUMNS_JP)
        else:             
            quote_df_out1['ComRevDivCd'] = pd.DataFrame(
                ComRevDivCd_orgi.split('-'))
            ...
            del quote_df_out1['ComRevDivCd']
            quote_df_out1 = pd.merge(quote_df_out1,ComRevDivCd_orgi2, on=['Componentid'])
            quote_df_out1 = pd.DataFrame(quote_df_out1, columns = REPORTER_COLUMNS)
