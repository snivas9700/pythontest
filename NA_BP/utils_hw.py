...
from modeling_utils import calc_quant_op, calc_win_prob, opt_price_ci, price_adj
from modeling_hw import calc_mf_opt
...
def build_comp_out(comp):
    ...
    comp_out['adj_price_L'] = comp_out['pred_L'] * comp_out['value_score']
    comp_out['adj_price_M'] = comp_out['pred_M'] * comp_out['value_score']
    comp_out['adj_price_H'] = comp_out['pred_H'] * comp_out['value_score']
...
def spread_comp_quants(comp, quote, source):
    dat = comp.copy()
    q = quote.copy()
    ...
    if source == 'online':
        ...
        # recalculate component-level OP, WP, CIs
        dat = dat.apply(lambda row: calc_mf_opt(row), axis=1)
