...
''' Harish - Bug Fix 1716354 ( No COPRA BP Optimal price )
    One more parameter added - source='train' to prep_comp function and ifelse clause added.
    This keeps the flow for offline code same. Only for online it skips the filtering part
'''
# only applies to component-level data for now
def prep_comp(data, sbc_map, source='train' ):
    dat = data.copy()
...
def prep_comp(data, sbc_map, source='train' ):
    ...
    dat.loc[:, 'ufc_incl'] = (data['ufc'].fillna('').apply(lambda x: x.strip()) != '') * 1
    ...
    if sum(sbc_filt) > 0:
        ...
        def find_max_sbc(row, sbc):
            if any([x in sbc.keys() for x in row.values]):
                match_keys = [x for x in row.values if x in sbc.keys()]
