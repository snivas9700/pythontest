...
def refresh_analysis(df):
    ...
    col_order = base_cols + rename_dict.values() + ['Revenue % Delta', 'switch']
    agg = DataFrame()
...
def calc_model_stats(vs, op):
    ...
    quad_dict = {quad_cnt.name + '_' + str(idx): val/float(op.shape[0]) for idx, val in quad_cnt.iteritems()}
...
def post_hoc_wrapper(train_vs, train_op, config, reg_folder, d, base_folder):
    ...
    folder_hash = hashlib.md5(config_str).hexdigest()[:10]  # unique for each distinct configuration
