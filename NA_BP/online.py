...
from NA_BP.data_prep import prep_comp, prep_quote
from NA_BP.online_modeling_hw import run_quants_online
from NA_BP.online_utils_hw import build_output, spread_comp_quants, transform_output
from NA_BP.utils import load_data

from NA_BP.modeling.segmentation_utils import find_seg_id
from NA_BP.modeling.modeling_main import apply_lme
...
def process_quote(df, loaded_models):
    ...
    region =  'NA' if (x == 'CA' or x == 'US') else 'JAPAN' if (x =='JP') else 'EMEA'
    ...
    quote_out = transform_output(comp_prep, tot_stats)
