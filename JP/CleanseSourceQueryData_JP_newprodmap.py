"""
    ...
    Last update:    12 June 2018 by Bonnie Bao
...
#data_path = '/Users/Zhu/Box Sync/Cognitive Pricing/COPRA_HW/WorkStreams/Model_development/Model_codes/JZ_version/Data/'
data_path = '~/Box Sync/Cognitive Pricing/COPRA_HW/WorkStreams/AP_GCG_LA_pricing/JP_pricing/data/training_data/050918/'
#file_in = 'HistoricalData_Italy_raw_01May16_30Apr17.csv'
#file_out = 'HistoricalData_Italy_01May16_30Apr17.csv'
#analysis_out = 'HistoricalData_Analysis_Italy_01May16_30Apr17.csv'
#QuotesOut = 'HistoricalData_QuotesOut_Italy_01May16_30Apr17.csv'

file_in = 'JP_trainingset_9MAY_v2.csv'
file_out = 'JP_trainingset_v2_cleanWL_051818.csv'
analysis_out = 'JP_trainingset_analysis_v2_WL_051818.csv'
QuotesOut = 'JP_trainingset_QuotesOut_v2_WL_051818.csv'

direct_training = False # when True, this resets all BP end user price as quoted price for BP line items
remove_all_losses = False # True removes all losses, False removes only duplicate losses
dup_cutoff = 0.2 #this is the threshold below which dupliates are deleted (used when remove_all_losses=False)
EMEA_SW_Cost_Cleanup = False #when True, this resets EMEA SW TMC costs in months before July 2017.  Code section C+
...
def report_update(report_df, OutputData, index_name):
    ...
    country_count = OutputData.loc[:, ('CountryCode', index_name)].groupby('CountryCode').sum().T
    country_count['Total'] = country_count.sum(axis=1)
    country_columns = pd.DataFrame(index=[index_name], columns=report_df.columns[:(len(report_df.columns)/3)]).fillna(0)
    ...
    delta_columns = [ '%s_%s' %(country, 'delta') for country in country_columns ]
    ...
    delta_count = pd.DataFrame([delta_values.tolist()], columns=delta_columns, index=country_count.index)

    pct_columns = [ '%s_%s' %(country, 'pct') for country in country_columns ]
    initial_records = report_df.loc['Initial Number of Records'][country_columns.columns]
    precentage = (delta_values / initial_records) * 100
    pct_df = pd.DataFrame([precentage.tolist()], columns=pct_columns, index=country_count.index)
...
print '     ', file_in
InputData = pd.read_csv(data_path + file_in, low_memory=False).fillna('')#,lineterminator='\n').fillna('') # add lineterminator = '\n', Yuxi 12/16/17
...
InputData['CountryCode'] = InputData['QT_COUNTRYCODE'].str.strip() # Edited by YC 11/03/17
InputData[initial_records] = 1
# Find the record count for each country
country_count = InputData[['CountryCode', initial_records]].groupby('CountryCode').sum().T
country_count['Total'] = country_count.sum(axis=1)
# Load the dataframe
delta_columns = OrderedDict( (country, '%s_%s' %(country, 'delta')) for country in country_count.columns )
delta_count = country_count.rename(columns = delta_columns)
pct_columns = [ '%s_%s' %(country, 'pct') for country in country_count.columns ]
pct_df = pd.DataFrame([['%s%%' % 100.00]*len(pct_columns)], columns=pct_columns, index=country_count.index)
...
OutputData['QuoteID'] = InputData['QT_QUOTEID'].astype(str).str.strip()  # remove blanks in text # Edit YC 11/03/17
OutputData['CountryCode'] = InputData['CountryCode'].str.strip()  # pull off the last 2 character in Quote ID (Country code)

#OutputData['ChannelID'] = InputData['QT_CHANNELID'].str.strip()  # remove blanks in text # Edit field name YC 11/03/17
OutputData['ChannelID'] = InputData['QT_CHANNELTYPE'].str.strip() 


OutputData['CustomerNumber'] = InputData['QT_CUSTOMERNB_CMR'].str.strip()  # remove blanks in text
OutputData['CRMSectorName'] = InputData['QTC_CMRSECTORNAME'].str.strip()  # remove blanks in text
OutputData['CRMIndustryName'] = InputData['QTC_CMRINDUSTRYNAME'].str.strip() # remove blanks in text
OutputData['DomBuyGrpID'] = InputData['DOM_BUY_GRP_ID'].str.strip()  # remove blanks in text
OutputData['DomBuyGrpNam'] = InputData['DOM_BUY_GRP_NAME'].str.strip()  # remove blanks in text
OutputData['OpportunityID'] = InputData['QT_OPPORTUNITYID'].str.strip()  # remove blanks in text
...
OutputData['ComListPrice'] = InputData['COM_LISTPRICE'] * InputData['COM_QUANTITY']
OutputData['ComTMC'] = InputData['COM_ESTIMATED_TMC'] * InputData['COM_QUANTITY']
# Edited by YC 11/13
OutputData['UnitComListPrice'] = InputData['COM_LISTPRICE']
OutputData['UnitComTMC'] = InputData['COM_ESTIMATED_TMC']

#Edited by Bonnie
OutputData['ComQuotePrice'] = InputData['COM_QUOTEPRICE'] * InputData['COM_QUANTITY']

if direct_training==True:
    OutputData.loc[OutputData.ChannelID == 'I','ComQuotePrice'] = InputData['COM_BPENDUSERPRICE']*InputData['COM_QUANTITY']

OutputData['ComDelgPriceL4'] = InputData['COM_DELGPRICEL4'] * InputData['COM_QUANTITY']

####### Edited by Bonnie#######
OutputData['ComQuantity'] = InputData['COM_QUANTITY']
#####################

OutputData['WinLoss'] = (InputData['QTW_WIN_IND'] == 'Y') * 1  # wins should be given a "1" value
OutputData['ComRevCat'] = InputData['COM_CATEGORY'].str.strip()  # remove blanks in text
#OutputData['ComRevDivCd'] = InputData['COM_RevDivCd'].str.strip()  # remove blanks in text
#OutputData['ComBrand'] = InputData['PRODUCT_BRAND'].str.strip()  # remove blanks in text
#OutputData['ComGroup'] = InputData['PRODUCT_GROUP'].str.strip()  # remove blanks in text
#OutputData['ComFamily'] = InputData['PRODUCT_FAMILY'].str.strip()  # remove blanks in text
...
OutputData['LVL4'] = InputData['LEVEL_4'].str.strip()
OutputData['LVL3'] = InputData['LEVEL_3'].str.strip()
OutputData['LVL2'] = InputData['LEVEL_2'].str.strip()
OutputData['LVL1'] = InputData['COM_MTM'].str.strip()

# Creating the Year column from QUOTE_DATE of InputData file
#Edited by Bonnie Bao
OutputData['Year'] = InputData['QT_APPROVALDATE'].astype(str).str[:4]
#OutputData['Year'] = pd.DatetimeIndex(InputData['QT_APPROVALDATE']).year
# Creating the month column from QUOTE_DATE of InputData file
OutputData['Month'] = InputData['QT_APPROVALDATE'].astype(str).str[4:6].astype(int)
#OutputData['Month'] = pd.DatetimeIndex(InputData['QT_APPROVALDATE']).month
# if Month=12,9,6,3 then EndOfQtr=1 else 0
OutputData['EndOfQtr'] = np.where(((OutputData['Month'].astype(int) % 3) == 0), 1, 0)
OutputData['ClientSegCd'] = InputData['QT_CLIENTSEGCD'].str.strip()  # remove blanks in text
# UpgradeFlag definition: 1=new, 3=cancellation, 4=MES, 5=Upg, C=RPO (Record Purposes Only)
# df.column_name = df.column_name.astype(np.int64)
OutputData['UpgMES'] = (InputData.COM_UPGMES.astype(str) >= '4') * 1  # ['COM_UPGRADEFLAG'] >= 4)*1 # Edited by YC need check?
# InputData['CLIENT_SEG_CD']='E' then 1 in below columns else 0
OutputData['ClientSeg=E'] = InputData['QT_CLIENTSEGCD'].str.strip().isin(['E']).astype(int)
...
OutputData['Indirect(1/0)'] = (OutputData['ChannelID'] == 'I') * 1
...
OutputData['Quote_approval_status'] = InputData['QT_APPROVALSTATUS'].str.strip() #added by Bonnie
...
if EMEA_SW_Cost_Cleanup == True:
    for i in range(len(OutputData.index)):
        date_calc = (int(OutputData.Year[i]) - 2015)*12 + int(OutputData.Month[i]) #this is the number of months after Dec 2015
        if (OutputData.ComRevCat[i] == 'S') and (date_calc < 31): # use 31 for SW components before Jul 2017
            test = OutputData.ComTMC[i] / OutputData.ComQuotePrice[i]
            if test <= .13: #if tmc/quote price < 13%, make TMC 2.5% of list price
                OutputData.set_value(i, 'ComTMC', OutputData.ComListPrice[i] * .025)
            elif (test > .13) and (test <= .5): #if tmc/quote price > 13% and <= 50%, make TMC 10% of list price
                OutputData.set_value(i, 'ComTMC', OutputData.ComListPrice[i] * .1)
            elif (test > .5) and (test <= .9): #if tmc/quote price > 50% and <= 90%, make TMC 25.6% of list price
                OutputData.set_value(i, 'ComTMC', OutputData.ComListPrice[i] * .256)
            else: #if tmc/quote price > 90%, make TMC 62% of list price
                OutputData.set_value(i, 'ComTMC', OutputData.ComListPrice[i] * .62)
...
#>> Remove records of quotes that were not approved
OutputData = OutputData[OutputData['Quote_approval_status'].isin(['Approved','Approval expired','Does not need Approval']) ]
report_df = report_update(report_df, OutputData, 'Removed all unapproved quotes')
print '          >records that were unapproved got removed'
...
OutputData = OutputData[((OutputData['ComRevCat'] == 'H') | (OutputData['ComRevCat'] == 'S'))]
...
report_df = report_update(report_df, OutputData, 'Remove records with missing customer numbers')
...
report_df = report_update(report_df, OutputData, 'Remove records with missing product hierarchy')
...
report_df = report_update(report_df, OutputData, 'Remove quote prices with >= 99% disc')
...
output_first['QuoteMonthID'] = 12 * (output_first.Year.astype(int) - 2015) + output_first.Month.astype(int)

quote_summary = output_first.loc[:, ('CountryCode', 'ChannelID', 'CustomerNumber', 'WinLoss', 'QuoteMonthID')]

com_quote_price = OutputData.loc[:, ('QuoteID', 'ComQuotePrice')].groupby('QuoteID').sum().rename(columns={'ComQuotePrice': 'TotalQuoteQuotedPrice'})
total_components = OutputData.loc[:, ('QuoteID', 'ComQuotePrice')].rename(columns={'ComQuotePrice': 'TotalQuoteComCount'})
total_components = total_components.groupby('QuoteID').count()

# New product map -- Yuxi Edited 12/16/17
#rev_div_cd = OutputData.loc[:, ('QuoteID', 'ComRevDivCd', 'ComQuotePrice')].groupby(('QuoteID', 'ComRevDivCd')).sum().reset_index()
#rev_div_cond = rev_div_cd.groupby('QuoteID')['ComQuotePrice'].transform(max) == rev_div_cd['ComQuotePrice']
#rev_div_cd = rev_div_cd[rev_div_cond].set_index('QuoteID').rename(columns={'ComRevDivCd': 'LeadRevDivCd', 'ComQuotePrice': 'LeadRevDivCdQuotePrice'})
#quote_summary = pd.concat([quote_summary, com_quote_price, total_components, rev_div_cd], axis=1)
...
level4 = level4[~level4.index.duplicated(keep='first')] #added by Bonnie

quote_summary = pd.concat([quote_summary, com_quote_price, total_components, level4], axis=1)
#quote_summary = quote_summary.drop_duplicates(subset=['QuoteID'])

quote_summary['DeleteCount'] = 1
quote_summary['Delete'] = 0

# New product map -- Yuxi Chen Edite 12/16/17
#quote_summary = quote_summary.reset_index().sort_values(['CountryCode', 'CustomerNumber', 'ChannelID', 'LeadRevDivCd', 'QuoteMonthID', 'QuoteID'])
...
# New product map -- Yuxi Chen Edite 12/16/17
#grouped = quote_summary.groupby(('CountryCode', 'CustomerNumber', 'ChannelID', 'LeadRevDivCd'))
...
for name, group in grouped:
    ...
    for i in range(len(group.index)):
        ...
        for index, row in group.iterrows():
            ...
            #                   + w2*(float(test_quote['LeadRevDivCdQuotePrice']) / float(base_quote['LeadRevDivCdQuotePrice'])-1)**2\
            ...
            if score < dup_cutoff: # a score below the dup_cutoff indicates that the older quote should be removed
                ...
                quote_summary.ix[index, 'DeleteCount'] += quote_summary.ix[base_index, 'DeleteCount']
                break
...
report_df = report_update(report_df, OutputData, 'Remove duplicate loss records')
...
summary_data['QuoteDate'] = OutputData['Month'].astype(str) + '-' + OutputData['Year'].astype('str')
...
min_date['QuoteDate'] = min_date['QuoteDate'].dt.month.map(str) + '-' + min_date['QuoteDate'].dt.year.map(str)
max_date['QuoteDate'] = max_date['QuoteDate'].dt.month.map(str) + '-' + max_date['QuoteDate'].dt.year.map(str)
min_date.loc['Total'] = ''
max_date.loc['Total'] = ''
min_date = min_date.rename(columns={'QuoteDate': 'Oldest Transaction Date'}).T
max_date = max_date.rename(columns={'QuoteDate': 'Youngest Transaction Date'}).T
summary_headers = report_df.columns[:len(report_df.columns)/3]

total_clean_win = OutputData.loc[:, ('CountryCode', 'WinLoss')].groupby('CountryCode').sum()
total_clean_win.loc['Total'] = total_clean_win.sum()

win_loss_pct_values = match_df(total_clean_win.loc[:, 'WinLoss'], report_df.ix[-1][summary_headers])
clean_win_percent = (win_loss_pct_values['df1'] / win_loss_pct_values['df2']) * 100
total_cleaned_quotes = OutputData.loc[:, ('QuoteID', 'CountryCode', 'WinLoss')].groupby(('CountryCode','QuoteID')).count().reset_index().loc[:, ('CountryCode', 'WinLoss')].groupby('CountryCode').count()
total_cleaned_quotes.loc['Total'] = total_cleaned_quotes.sum()
cleaned_win_quotes = OutputData.loc[:, ('CountryCode', 'WinLoss', 'QuoteID')].groupby(('CountryCode', 'QuoteID')).sum().reset_index().loc[:, ('CountryCode', 'WinLoss')]
cleaned_win_quotes['WinLoss'] = np.where((cleaned_win_quotes['WinLoss'] > 0), 1, 0)
cleaned_win_quotes = cleaned_win_quotes.groupby('CountryCode').sum()
cleaned_win_quotes.loc['Total'] = cleaned_win_quotes.sum()
cleaned_win_percent = (cleaned_win_quotes.astype(float)/ total_cleaned_quotes.astype(float)) * 100
cleaned_win_percent = cleaned_win_percent.applymap(lambda x: '%.2f%%' %x)
total_clean_win = total_clean_win.rename(columns={'WinLoss': 'Total # of cleaned win records'}).T
total_cleaned_quotes = total_cleaned_quotes.rename(columns={'WinLoss': 'Total # of cleaned quotes'})
cleaned_win_quotes = cleaned_win_quotes.rename(columns={'WinLoss': 'Total # of cleaned win quotes'})
cleaned_win_percent = cleaned_win_percent.rename(columns={'WinLoss': 'Cleaned win quote percent'})
total_clean_win = total_clean_win.applymap(lambda x: "{:,}".format(x))
total_cleaned_quotes = total_cleaned_quotes.applymap(lambda x: "{:,}".format(x))
...
total_clean_win.loc[cwp_text] = total_clean_win.loc[cwp_text].apply(lambda x: '%.2f%%' %x)
summary_stats = pd.concat([min_date, max_date, total_clean_win, total_cleaned_quotes.T, cleaned_win_quotes.T, cleaned_win_percent.T])
...
report_df[num_columns] = report_df[num_columns].applymap(lambda x: "{:,}".format(x))
