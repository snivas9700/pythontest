...
from JP.modeling.modeling_utils import find_model, apply_model, calc_win_prob, calc_bounds, apply_bounds, calc_profit
from JP.modeling.segmentation_utils import navigate_tree
...
def apply_lme(row, m_dict):
    ...
    if 'mixed_effects' in list(m_dict.keys()):
        ...
        vs = min(vs,.9999)   #Not capped at 1 to avoid zero division error
        vs = max (0.0,vs)
