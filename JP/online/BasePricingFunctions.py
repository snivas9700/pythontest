"""
This file defines the Base Optimal Pricing Functions
"""
...
def PriceAdj(low, med, high):
    ...
    """
     Written by Glenn Melzer on:  22 Feb 2016
      Last update:                21 Apr 2017 by Glenn Melzer
    ...
    # print 'skew: ', skew
    if (skew < min_skew):
        high = np.maximum(high, ((np.minimum((2 * med) - low, 0.95)) + high) / 2)
...
def OptPriceLogisticReg(B0, B1, Cf=0, Cv=0, FCw=0, FCl=0):
    """B0=Logit Regression B0, B1=Logit Regression B1, Cf=fixed cost, Cv=variable cost(%ofnet), FCw=fin contrib if win, FCl=fin contrib if loss"""
    """
    Written by Glenn Melzer on:   04 Nov 2016
      Last update:                05 Nov 2016

    This function calculates the optimal price by maximizing the expected financial
    contribution of a deal.  The probability of winning function uses the logistic
    formula:
                                 1
    Win Probability = ---------------------
                       1 + exp(-(B0 + B1*X))    Where "exp" is e raised to a power

    The financial contribution is based on price minus fixed and variable costs.
    There is also an adjustment available for follow-on incremental expected
    financial contribution of winning and loosing.

    INPUT:
      B0  (the B0 parameter in the logistic function)
      B1  (the B1 parameter in the logistic function)
      Cf  (the fixed cost)
      Cv  (the variable cost, as a function of the price)
      FCw (the incremental financial contribution if the deal WINS)
      FCl (the incremental financial contribution if the deal LOSES)
    ...
    iterations = 8
    slices = 10
    percentile = .995  # this is the percentile associated with starting the search from the low end
    Start = (-math.log(1 / percentile - 1) - B0) / B1  # start search at this price
    Finish = (-math.log(1 / (1 - percentile) - 1) - B0) / B1  # finish search at this price
    for i in range(iterations):
        GPMax = -99999999.0
        POpt = -99999999.0
        d = (Finish - Start) / slices
        # print Start, Finish, d
        for P in np.arange(Start, Finish, d):
            EGP, Q = ExpFCLogisticReg(P, B0, B1, Cf, Cv, FCw, FCl)
            if EGP > GPMax:
                GPMax = EGP
                POpt = P
        Start = POpt - d
...
def ExpFCLogisticReg(P, B0, B1, Cf=0, Cv=0, FCw=0, FCl=0):
    """P=price input, B0=Logit Regression B0, B1=Logit Regression B1, Cf=fixed cost, Cv=variable cost(%ofnet), FCw=fin contrib if win, FCl=fin contrib if loss"""
    """
    Written by Glenn Melzer on:   04 Nov 2016
    ...
    This function calculates the expected financial contribution of a particular
    price given the fixed cost, the variable cost,the probability of winning
    (based on the logistic regression formula), and the follow-on incremental
    expected financial contribution of winning and losing.  The logistic
    win probability formula is:
                                 1
    Win Probability = ---------------------
                       1 + exp(-(B0 + B1*X))    Where "exp" is e raised to a power
