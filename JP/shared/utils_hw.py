...
from JP.shared.modeling_hw import calc_mf_opt
from JP.shared.modeling_utils import calc_quant_op, calc_win_prob, opt_price_ci, price_adj
...
def build_tot_stats(comp_out):
    ...
    stats['opt_wp'] = 0.
    stats['value_score'] = grp['value_score'].sum().round(decimals=2) #added by BB June 19th, 2018
...
def spread_optimal_price(quote_df, tot_deal_stats):
    ...
    tots = merge(tots, tot_deal_stats.reset_index()[['bot_OP', 'quoteid','value_score']], on='quoteid', how='inner')

    #quote level bottom line optimal price will not be lower than 80% of value score for BP and 90% of value score Direct quotes
    if (quote_df.indirect.values[0]==1):
        print('testing BP quotes')
        tots['bot_OP'] = tots['bot_OP'].mask(tots.bot_OP < 0.8*tots.value_score,0.8*tots.value_score) 
    elif (quote_df.indirect.values[0]==0):
        print('testing Direct quotes')
        tots['bot_OP'] = tots['bot_OP'].mask(tots.bot_OP < 0.9*tots.value_score,0.9*tots.value_score)
