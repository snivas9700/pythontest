...
def process_model_entry(m_def):
    parsed = []
    for param, weight in m_def.items():
        feat = {}
        f_list = []
...
def coalesce_trees(tree_dict):
    ...
    for i, k in enumerate(sorted(list(tree_dict.keys()), key=len)):
        tree = tree_dict[k]
        if i == 0:
            out_dict = tree
        else:
...
def coalesce(parent, child):
    ...
    valid_keys = [z for z in list(parent.keys()) if z not in ['inputs', 'target']]
...
def model_entry_to_dict(model_entry, seg_dict, tier_map, m_dict, tier_dict):
    ...
    feat_names = dict(list(zip(feat_cols, clean_feat_cols)))
...
def quote_model_to_dict(df, seg_dict, tier_map, level):
    ...
    range_tiers = [k for k in list(tier_map.keys()) if tier_map[k]['type'] not in ['categorical', 'discrete', 'binary']]
    ...
    tier_df = tier_df.drop(list(tier_names.values()), axis=1).reset_index().rename(columns={'index': 'segment_id'})
...
def apply_tier_map(data, train_qt, tier_map):
    ...
    # build tier values
    for i, (k, v) in enumerate(tier_map.items()):
